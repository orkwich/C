...
int separate_negativ_and_positiv(int tab[], int size)
{
    int position = 0;
...
{
    int pivot = separate_negativ_and_positiv(tab, size);
    if(pivot == 0)
        return quicksort_desc(0, size - 1, tab, size);
    if(pivot == size - 1)
        return quicksort_asc(0, size - 1, tab, size);
    return !(!quicksort_asc(0, pivot, tab, size) && !quicksort_desc(pivot + 1, size - 1, tab, size));
}
...
int main(void)
{
    ...
    sort_negative_asc_positive_desc(tab, size);
